...
describe('TokenManager (browser)', function() {
  ...
  describe('dpop', function () {
    let ctx: any = {};

    beforeEach(async () => {
      ctx.client = await setup({ dpop: true });
      const onErrorSpy = jest.fn();
      ctx.onErrorSpy = onErrorSpy;
      ctx.client.tokenManager.on('error', onErrorSpy);

      // load a dpop KP into storage, to test it's removal upon token removal
      await dpop.getDPoPKeyPair();
    });
    ...
    it('removes dpop KP from storage when a DPoP acquired access token is removed', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      accessToken.tokenType = 'DPoP';
      ctx.client.tokenManager.add('test-accessToken', accessToken);

      expect(await dpop.loadKeyPair()).not.toEqual(null);
      ctx.client.tokenManager.remove('test-accessToken');
      expect(ctx.onErrorSpy).not.toHaveBeenCalled();
      expect(await dpop.loadKeyPair()).toEqual(null);
    });

    it('retains dpop KP when a non-DPoP acquired access token is removed', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      // Ref: accessToken.tokenType = Bearer
      ctx.client.tokenManager.add('test-accessToken', accessToken);
    ...
    it('retains dpop KP when an access token is removed but refresh token exists', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      accessToken.tokenType = 'DPoP';
      ctx.client.tokenManager.add('test-accessToken', accessToken);
      ctx.client.tokenManager.add('test-refreshToken', tokens.standardRefreshTokenParsed);
    ...
    it('retains dpop KP when a ID token is removed', async () => {
      const idToken = {...tokens.standardIdTokenParsed};
      ctx.client.tokenManager.add('test-idToken', idToken);

      expect(await dpop.loadKeyPair()).not.toEqual(null);
      ctx.client.tokenManager.remove('test-idToken');
      expect(ctx.onErrorSpy).not.toHaveBeenCalled();
      expect(await dpop.loadKeyPair()).not.toEqual(null);
    });

    it('removes dpop KP from storage when a DPoP acquired refresh token is removed', async () => {
      ctx.client.tokenManager.add('test-refreshToken', tokens.standardRefreshTokenParsed);

      expect(await dpop.loadKeyPair()).not.toEqual(null);
      ctx.client.tokenManager.remove('test-refreshToken');
      expect(ctx.onErrorSpy).not.toHaveBeenCalled();
      expect(await dpop.loadKeyPair()).toEqual(null);
    });

    it('retains dpop KP when an refresh token is removed but access token exists', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      accessToken.tokenType = 'DPoP';
      ctx.client.tokenManager.add('test-accessToken', accessToken);
      ctx.client.tokenManager.add('test-refreshToken', tokens.standardRefreshTokenParsed);
    ...
    // NOTE: reason for sleep() - testing un-awaited promise behavior
    // clearDPoPKeyPair() is called to clear the DPoP key pair from storage, this method
    // is required to be async, however it is *NOT* awaited. This test needs to pause for
    // a short duration so the promise has time to reject (and emit the error) before the
    // test exit. This test confirms the error will be emitted as expected
    it('should emit error (not throw) when clearDPoPKeyPair throws error', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      accessToken.tokenType = 'DPoP';
      ctx.client.tokenManager.add('test-accessToken', accessToken);

      jest.spyOn(dpop, 'clearDPoPKeyPair').mockRejectedValueOnce(new Error('Test Error'));

      expect(await dpop.loadKeyPair()).not.toEqual(null);
      ctx.client.tokenManager.remove('test-accessToken');
      await util.sleep(100);    // [REQUIRED] see not above test
      expect(ctx.onErrorSpy).toHaveBeenCalled();
    });

    it('should clear dpop PK after .clear', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
      accessToken.tokenType = 'DPoP';
      ctx.client.tokenManager.add('test-accessToken', accessToken);
      ctx.client.tokenManager.add('test-refreshToken', tokens.standardRefreshTokenParsed);
      ...
      expect(await dpop.loadKeyPair()).not.toEqual(null);
      ctx.client.tokenManager.clear();
      expect(ctx.onErrorSpy).not.toHaveBeenCalled();
      expect(await dpop.loadKeyPair()).toEqual(null);
    });

    it('retains dpop KP when .clear is called but token type is not DPoP', async () => {
      const accessToken = {...tokens.standardAccessTokenParsed};
